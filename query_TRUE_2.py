...
def check_ollama_model() -> bool:
    """
    Query /api/tags to confirm OLLAMA_MODEL is available locally.
    Returns True if OK, prints a helpful message and returns False otherwise.
    """
    try:
    ...
    if OLLAMA_MODEL not in available:
        print(f"\n  ✗  Model '{OLLAMA_MODEL}' is not pulled yet.")
...
def call_ollama(system: str, user: str) -> str:
    """
    Stream /api/chat token-by-token.

    Why streaming fixes the timeout:
      stream=False  → requests waits for the *entire* response before returning.
                      On slow hardware this easily exceeds any fixed timeout.
      stream=True   → Ollama flushes one JSON line per token.
                      timeout=(connect_s, read_s) applies to each individual
                      read(), not to the full generation, so the clock resets
                      with every token and the request never times out mid-run.

    Tokens are printed to stdout in real time so you see the answer build up.
    """
    ...
    payload = {
        ...
        "options": {
            ...
            "num_ctx": 4096,                 # 4 k fits 10 chunks comfortably
        },
        "messages": [
    ...
    try:
        resp = _requests.post(
            url,
            json=payload,
            timeout=(10, 120),               # (connect_timeout, per-read_timeout)
    ...
    print(f"\n{SEP}")
    print(f"  {OLLAMA_MODEL}  (streaming)")
    print(SEP + "\n")
...
def main():
    ...
    # Verify Ollama + model before loading the heavy embedding model
    if LLM_PROVIDER == "ollama" and not check_ollama_model():
        return

    chunks, index, model = load_resources()
    print(f"\n  Ready.  {len(chunks)} chunks | {index.ntotal} FAISS vectors | top-k = {TOP_K}")
    print(f"  LLM: {OLLAMA_MODEL} via Ollama (streaming)")
    ...
    while True:
        ...
        # For Ollama: call_ollama() already streams the answer to stdout
        #             so we just collect the return value for any post-use.
        # For other providers: print_answer() formats and prints the reply.
